...
# Base directories
# ---------------------------------------------------------- #
PROJECT_DIR="/home/zuckram/Studium/EP1/Uebung_6/Aufgabenblatt5/"
# ---------------------------------------------------------- #
...
# ---------------------------------------------------------- #
SOURCE_FILES="$SRC_DIR/Aufgabe3.java"
...
awk '/Test run finished/,/\[ [0-9]+ tests failed \]/ { next } { print }' "$OUTPUT_FILE" > temp_file && mv temp_file "$OUTPUT_FILE"
#
#awk '!/Thanks for using JUnit/ && !/^$/ && !/^╷$/ && !/Test run finished/ && !/\[.\]/' "$OUTPUT_FILE" > temp_file && mv temp_file "$OUTPUT_FILE"
#
...
java -Xss16m -cp "$RUNTIME_CLASSPATH" "$RUN_FILES" "$@"
